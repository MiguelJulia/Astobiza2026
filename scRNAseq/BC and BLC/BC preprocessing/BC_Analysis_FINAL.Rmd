...
author: "Lena Cords (modified by Miguel Juliá)"
output:
  html_document:
    df_print: paged
---
...
```{r}
set.seed(101100)
setwd("/path/to/working/directory/scRNAseq_test/Cords_2023/BodenmillerGroup/CAFclassification-v0.1.1/BodenmillerGroup-CAFclassification-09b0205/scRNA-seq/")
```
...
data.folder <- file.path(wd,"data","breast")

list_of_files <- list.files("E-MTAB-10607/", pattern = "count_matrix.txt")
list_of_files_metadata <- list.files("E-MTAB-10607/", pattern = "metadata.txt")

# # keep only non-exausted samples
# list_of_files <- c("TBB035_singlecell_count_matrix.txt", "TBB075_singlecell_count_matrix.txt", "TBB102_singlecell_count_matrix.txt", "TBB212_singlecell_count_matrix.txt", "TBB214_singlecell_count_matrix.txt", "TBB226_singlecell_count_matrix.txt", "TBB330_singlecell_count_matrix.txt")
# list_of_files_metadata <- c("TBB035_complete_singlecell_metadata.txt", "TBB075_complete_singlecell_metadata.txt", "TBB102_complete_singlecell_metadata.txt", "TBB212_complete_singlecell_metadata.txt", "TBB214_complete_singlecell_metadata.txt", "TBB226_complete_singlecell_metadata.txt", "TBB330_complete_singlecell_metadata.txt")

for (file in 1:length(list_of_files) ){
  
  counts <- read.table(paste0("E-MTAB-10607/", list_of_files[file]), sep="\t", header = T)
  metadata <- read.table(paste0("E-MTAB-10607/", list_of_files_metadata[file]), sep="\t", header = T)
  sample <- metadata$sample[1]
  metadata$cellname <- paste0(gsub(paste0(sample, "_"), "", metadata$cellID), ".1")
  
  srat <- CreateSeuratObject(counts,  project = sample)
  
  srat@meta.data$sample <- sample
  srat@meta.data$cell_type <- NA
  for (i in 1:length(metadata$cell_type)){
    which(metadata$cellname[i] == Cells(srat))
    srat@meta.data$cell_type[which(metadata$cellname[i] == Cells(srat))] <- metadata$cell_type[i]
  }
  ...
  # Save variables
  assign(paste0("srat_", sample), srat)
...
# integration_features <- SelectIntegrationFeatures(object.list=c(srat_TBB035, srat_TBB075, srat_TBB102, srat_TBB212, srat_TBB214, srat_TBB226, srat_TBB330)) 
# 
# srat.anchors <- FindIntegrationAnchors(object.list = c(srat_TBB035, srat_TBB075, srat_TBB102, srat_TBB212, srat_TBB214, srat_TBB226, srat_TBB330), anchor.features = integration_features)

integration_features <- SelectIntegrationFeatures(object.list=c(srat_TBB011, srat_TBB035, srat_TBB075, srat_TBB102, srat_TBB111, srat_TBB129, srat_TBB165, srat_TBB171, srat_TBB184, srat_TBB212, srat_TBB214, srat_TBB226, srat_TBB330)) 

srat.anchors <- FindIntegrationAnchors(object.list = c(srat_TBB011, srat_TBB035, srat_TBB075, srat_TBB102, srat_TBB111, srat_TBB129, srat_TBB165, srat_TBB171, srat_TBB184, srat_TBB212, srat_TBB214, srat_TBB226, srat_TBB330), anchor.features = integration_features)
...
# Save results
save.image("BC_data_RAW.RData")  

# Only srat.combined
load("/home/juliam/Documents/FernandoCalvo/scRNAseq_test/Signaures/Breast.srat.combined.RData")
...
#all.fibroblast <- readRDS(file = file.path( paste( "fibroblasts-clustering.RDS")))

#load final data including clustering and phenotype assignment
#all.fibroblast <-readRDS(file=file.path( paste("BC_data.RDS")))

# Download data from https://www.ebi.ac.uk/biostudies/arrayexpress/studies/E-MTAB-10607


saveRDS(all.fibroblast, file=file.path( paste("BC_data_RAW.RDS")))

#load processed data
#all.fibroblast <- readRDS(file=file.path( paste("fibroblasts-clustering_final.RDS")))
#saveRDS(all.fibroblast, file=file.path( paste("fibroblasts-clustering_final.RDS")))
```
-->

```{r, get all fibroblasts}

srat.combined <- SetIdent(srat.combined, value = "seurat_clusters")
DimPlot(srat.combined, reduction = "umap", label = TRUE, repel = TRUE)

markers_all_seurat <- FindAllMarkers(srat.combined)
write.csv(markers_all_seurat, file="markers_all_seurat.csv")

# # Cell type annotation using SingleR
# library(SingleR)
# hpca.ref <- celldex::HumanPrimaryCellAtlasData()
# sce <- as.SingleCellExperiment(DietSeurat(JoinLayers(srat.combined)))
# sce
...
# # sce@meta.data$hpca.fine   <- hpca.fine$pruned.labels
# 
# srat.combined <- SetIdent(srat.combined, value = "hpca.main")
# DimPlot(srat.combined, label = T , repel = T, label.size = 3) + ggtitle('Tissue types SigneR')
# 
# srat.combined <- SetIdent(srat.combined, value = "hpca.main")
...
srat.combined <- SetIdent(srat.combined, value = "cell_type")
DimPlot(srat.combined, label = T , repel = T, label.size = 3) + ggtitle('Tissue types metadata')

srat.combined <- SetIdent(srat.combined, value = "cell_type")
list_fibroblasts <- WhichCells(srat.combined, idents = c( "fibroblast"))
DimPlot(srat.combined, label=T, cells.highlight= list(list_fibroblasts), cols.highlight = c("darkblue"), cols= "grey")

markers_all_celltype <- FindAllMarkers(srat.combined)
write.csv(markers_all_celltype, file="markers_all_celltype.csv")

srat.combined <- SetIdent(srat.combined, value = "sample")
DimPlot(srat.combined, label = T , repel = T, label.size = 3) + ggtitle('Sample origin')

srat.combined <- SetIdent(srat.combined, value = "cell_type")
all.fibroblast <- subset(x = srat.combined, ident = c("fibroblast"))

# Run the standard workflow for visualization and clustering
all.fibroblast <- ScaleData(all.fibroblast, verbose = FALSE)
all.fibroblast <- RunPCA(all.fibroblast, npcs = 30, verbose = FALSE)
all.fibroblast <- RunUMAP(all.fibroblast, reduction = "pca", dims = 1:30)
all.fibroblast <- FindNeighbors(all.fibroblast, reduction = "pca", dims = 1:30)
all.fibroblast <- FindClusters(all.fibroblast, resolution = 0.5)

DimPlot(all.fibroblast, reduction = "umap", label = TRUE, repel = TRUE)
...
# ggsave(p, file=file.path( "RAW_run_sample_henotype_BC_UMAPs.png"), width=18, height=6)
# 
# saveRDS(all.fibroblast, file=file.path( "fastMM_corected_BC.rds"))
```
FastMM corrected
```{r, fig.width=18, fig.height=6}
data_folder <- file.path(wd,"data")

all.fibroblast <-readRDS(file=file.path( "fastMM_corected_BC.rds"))
...
plot(p)
saveRDS(all.fibroblast.h, file=file.path( "harmony_corected_BC_dim20.rds"))
...
p <- DimPlot(all.fibroblast.h, group.by = c("run", "phenotype_BC","Sample"))+scale_color_tableau("Tableau 20")
plot(p)
saveRDS(all.fibroblast.h, file=file.path( "harmony_corected_BC_dim25.rds"))
...
saveRDS(all.fibroblast.h, file=file.path( "harmony_corected_BC_dim25.rds"))
all.fibroblast.h <-readRDS(file=file.path( "harmony_corected_BC.rds"))
```
```{r, fig.width=18, fig.height=6}
all.fibroblast.h <-readRDS(file=file.path("harmony_25","harmony_fibroblasts_BC_clustered_dims25.rds"))
...
all.fibroblast.h <- all.fibroblast
...
#UMAP & TSNE
# all.fibroblast.h <- RunUMAP(object = all.fibroblast.h, dims = 1:25, reduction = "harmony", assay.use = "integrated")
# all.fibroblast.h <- RunTSNE(object = all.fibroblast.h, dims = 1:25, reduction = "harmony", assay.use = "integrated")
all.fibroblast.h <- RunUMAP(object = all.fibroblast.h, dims = 1:25)
all.fibroblast.h <- RunTSNE(object = all.fibroblast.h, dims = 1:25)
```

```{r, UMAPs 1}
#Samples
p<-DimPlot(object = all.fibroblast.h, reduction = 'umap', group.by = "orig.ident") +labs(colour="Sample ID")
p
...
all.fibroblast.h <- FindNeighbors(object = all.fibroblast.h, dims = 1:25) %>% FindClusters(resolution = seq(from = 0.1, to = 2, by = 0.1))
all.fibroblast.h <- FindClusters(object = all.fibroblast.h, resolution = seq(from = 0.1, to = 1, by = 0.1))

all.fibroblast.h <- FindNeighbors(object = all.fibroblast.h, dims = 1:25, graph.name = "SCT_snn") %>% FindClusters(resolution = seq(from = 0.1, to = 2, by = 0.1), graph.name = "SCT_snn")
all.fibroblast.h <- FindClusters(object = all.fibroblast.h, resolution = seq(from = 0.1, to = 1, by = 0.1), graph.name = "SCT_snn")
```

## Clustree analysis
```{r, fig.width=12, fig.height=12}
p <- clustree(all.fibroblast.h, prefix = "integrated_snn_res.0", exprs = "scale.data")
plot(p)

#ggsave(filename=file.path( paste("clustree_allfibroblasts_harmony.pdf")), plot=p, width=10, height=10)
```
...
write.csv(harmony_markers_mast_activation_state, file = file.path(paste( "harmony_harmony_markers_mast_activation_state1.csv")), row.names = FALSE)
...
write.csv(harmony_markers_mast_activation_state_top10, file = file.path(paste0( "harmony_markers_mast_activation_state_top20.csv")), row.names = TRUE)

p <-DoHeatmap(all.fibroblast.h, features=c("FAP",harmony_markers_mast_activation_state_top20$gene)) + NoLegend()+scale_fill_viridis()
p
ggsave(p, file=file.path( paste("harmony_HM_top10_activation_state.pdf")), width=8, height=16)


```

```{r}
data_folder <- file.path( "harmony_25")
...
write.csv(harmony_markers_mast_res01, file = file.path(paste( "harmony_harmony_markers_mast_res01.csv")), row.names = FALSE)

harmony_markers_mast_res01_top10 <- harmony_markers_mast_res01 %>% group_by(cluster) %>% top_n(n = 10, wt = avg_log2FC)
write.csv(harmony_markers_mast_res01_top10, file = file.path(paste0( "harmony_markers_mast_res01_top10.csv")), row.names = TRUE)
```
...
harmony_markers_mast_res01_top10 <- read.csv(file = file.path("harmony_25",paste0( "harmony_markers_mast_res01_top10.csv")))
...
write.csv(harmony_markers_mast_res04, file = file.path("harmony_25",paste( "harmony_harmony_markers_mast_res04.csv")), row.names = FALSE)

harmony_markers_mast_res04_top10 <- harmony_markers_mast_res04 %>% group_by(cluster) %>% top_n(n = 10, wt = avg_log2FC)
write.csv(harmony_markers_mast_res04_top10, file = file.path(paste0( "harmony_markers_mast_res04_top10.csv")), row.names = TRUE)
```
...
harmony_markers_mast_res04_top10 <- read.csv(file = file.path("harmony_25",paste0( "harmony_markers_mast_res04_top10.csv")))
...
VlnPlot(all.fibroblast.h, features=c("FAP","ACTA2"), split.by = "CAFtype")

# saveRDS(all.fibroblast.h, file=file.path("harmony_fibroblasts_BC_clustered_dims25.rds"))
# all.fibroblast.h <-readRDS(file=file.path("harmony_25","harmony_fibroblasts_BC_clustered_dims25.rds"))
# all.fibroblast.h <-readRDS(file=file.path("harmony_fibroblasts_BC_clustered_dims25.rds"))
# 
# all.fibroblast.h2 <-readRDS(file=file.path("harmony_fibroblasts_BC_clustered.rds"))
```
...
<!-- VlnPlot(all.fibroblast.h, features=c("FAP","ACTA2"), split.by = 'SCT_snn_res.0.4') -->

<!-- table(all.fibroblast.h$SCT_snn_res.0.1) -->
...
plot(p)
#ggsave(p, file=file.path( "harmony25_all_CAFtype.png"))
```
...
<!-- #ggsave(plot=p, file=file.path("Sankey_old_new_clustering_minusCluster.pdf")) -->
<!-- #ggsave(plot=p, file=file.path("Sankey_old_new_clustering_minusCluster.png"), width=20, height=10) -->
...
p<-
  ...
  xlab("Phenotype")
p
#ggsave(filename=file.path( paste("CellNumber_phenotype_new-clusters.pdf")), plot=p)

```
#Prop new
```{r, Phenotypes per patients barplot 1}
#new
...
p <-
  ...
  scale_fill_tableau()
p
#ggsave(file=file.path(paste0('Barplot_phenotype_prop_per_patient_new-clusters.pdf')))
```
...
write.csv(harmony_markers_mast_CAFtype, file = file.path(paste( "harmony_harmony_markers_mast_CAFtype.csv")), row.names = FALSE)
harmony_markers_mast_CAFtype <- read.csv(file = file.path(paste( "harmony_harmony_markers_mast_CAFtype.csv")))

harmony_markers_mast_CAFtype_top10 <- harmony_markers_mast_CAFtype %>% group_by(cluster) %>% top_n(n = 10, wt = avg_log2FC)
write.csv(harmony_markers_mast_CAFtype_top10, file = file.path(paste0( "harmony_markers_mast_CAFtype_top10.csv")), row.names = TRUE)

harmony_markers_mast_CAFtype_top6 <- harmony_markers_mast_CAFtype %>% group_by(cluster) %>% top_n(n = 6, wt = avg_log2FC)
write.csv(harmony_markers_mast_CAFtype_top6, file = file.path(paste0( "harmony_markers_mast_CAFtype_top6.csv")), row.names = TRUE)

p <-DoHeatmap(all.fibroblast.h, features=harmony_markers_mast_CAFtype_top6$gene) + NoLegend()+scale_fill_viridis()
plot(p)
#ggsave(p, file=file.path( paste("HM_top10_phenotype_harmony_final.png")), width=12, height=12, dpi=300)

ggsave(p, file=file.path(paste("HM_top10_phenotype_harmony_final_top6.pdf")), width=12, height=18)
```
...
print(tbl)
write.csv(tbl, file=file.path(paste("table_numbers-per-phenotype-BC-harmony.csv")),row.names = F)
```
...
plot(p)
ggsave(filename=file.path(paste("UMAP_FeaturePlot_harmony_all.png")), plot=p, width=8, height=12, dpi=300)

#FAP and PDGFRa
p <-FeaturePlot(all.fibroblast.h, features=c("FAP","PDGFRA"), cols=c("lightgrey","black")) &NoAxes()
plot(p)
ggsave(filename=file.path(paste("UMAP_FeaturePlot_harmony_all_FAP_PDGFRa.png")), plot=p, width=6, height=4, dpi=300)

```



```{r, Phenotypes per patients barplot 2}
df <- prop.table(x=table(all.fibroblast.h$CAFtype, all.fibroblast.h$orig.ident), margin=2)
df <- as.data.frame(df)
colnames(df) <- c('Type', 'Sample', 'Prop')
...
p <-
  ...
  scale_fill_tableau()
p
ggsave(file=file.path(paste0('Barplot_CAFtype_prop_patients_tableau_BC.pdf')))
...
p <-
  ...
  scale_fill_tableau()
p
ggsave(file=file.path(paste0('Barplot_CAFtype_N_patients_tableau_BC.pdf')))
...
plot(p)
ggsave(plot=p, file=file.path("HM_average_1marker.pdf"))
```
...
plot(p)
ggsave(filename=file.path( paste("UMAP_RAW_Featureplot_CD34_POSTN.png")), plot=p, width=18, height=8, dpi=300)


p <-FeaturePlot(all.fibroblast.h, features=c(my_genes,"ACTA2"), cols=c("lightgrey","black"), ncol=8) &NoAxes()
plot(p)
ggsave(filename=file.path( paste("UMAP_harmony_Featureplot_CD34_POSTN.png")), plot=p, width=18, height=8, dpi=300)
...
<!-- saveRDS(subsample, file=file.path( "subsample_harmony_BC_CD34incl.rds")) -->
<!-- #build graph -->
...
<!-- saveRDS(subsample, file=file.path( "subsample_harmony_BC_CD34incl_clustered.rds")) -->

<!-- ``` -->


<!-- ```{r, Heatmap for differentially expressed genes per cluster marker genes 1} -->
...
<!-- ```{r, Heatmap for differentially expressed genes per cluster marker genes 2} -->
...
<!-- #ggsave(p, file=file.path(results.folder, paste("HM_phenotypes_sub_MARKER_allcells.png")), width=10, height=10) -->
<!-- ``` -->
<!-- ```{r, Heatmap for differentially expressed genes per cluster marker genes 3} -->
...
<!-- #ggsave(p, file=file.path(results.folder, paste("HM_phenotypes_sub_MARKER_allcells.png")), width=10, height=10) -->
<!-- ``` -->
<!-- ```{r, Heatmap for differentially expressed genes per cluster marker genes 4} -->
...
<!-- ```{r, Heatmap for differentially expressed genes per cluster marker genes 5} -->
...
<!-- ```{r, Heatmap for differentially expressed genes per cluster marker genes 6} -->
...
<!-- ggsave(plot=p, file=file.path( "HM_MARKERgenes_clustering.pdf")) -->
<!-- ``` -->
...
<!-- plot(p) -->
<!-- ggsave(plot=p, file=file.path("FeaturePlot_IMCgenes.png"), width=12, height=12, dpi = 300, units = "in") -->
<!-- ``` -->
...
<!-- ``` -->
<!-- ```{r, Heatmap for differentially expressed genes per cluster for IMC genes 7} -->
...
<!-- ```{r, Heatmap for differentially expressed genes per cluster for IMC genes 8} -->
...
<!-- #ggsave(p, file=file.path(results.folder, paste("HM_phenotypes_sub_MARKER_allcells.png")), width=10, height=10) -->
<!-- ``` -->
<!-- ```{r, Heatmap for differentially expressed genes per cluster for IMC genes 9} -->
...
<!-- ```{r, Heatmap for differentially expressed genes per cluster for IMC genes 10} -->
...
<!-- ```{r, Heatmap for differentially expressed genes per cluster for IMC genes 11} -->
...
<!-- ```{r, Heatmap for differentially expressed genes per cluster for IMC genes 12} -->
...
<!-- ```{r, Heatmap for differentially expressed genes per cluster for IMC genes 13} -->
...
<!-- #ggsave(p, file=file.path(results.folder, paste("HM_phenotypes_sub_MARKER_allcells.png")), width=10, height=10) -->
<!-- ``` -->
<!-- ```{r, Heatmap for differentially expressed genes per cluster for IMC genes 14} -->
...
<!-- p -->
<!-- ggsave(plot=p, file=file.path( "HM_IMCgenes_clustering.pdf")) -->
<!-- ``` -->
...
<!--  # ggtitle("passengers on the maiden voyage of the Titanic") -->
<!-- plot(p) -->
<!-- ggsave(plot=p, file=file.path("SankeyPlot_allfeatures_vs_IMCgene_clustering.pdf")) -->
<!-- ``` -->
...
<!-- #pheatmap -->
<!-- pdf(file=file.path( paste0("FINAL_Clustering_IMCgenes_phenotype_final_HM_statsSankey.pdf")), width=10, height=10) -->

<!-- pheatmap(df.hm.p, display_numbers = T, fontsize=25) -->
<!-- dev.off() -->

<!-- pdf(file=file.path( paste0("FINAL_Clustering_IMCgenes_phenotype_final_HM_statsSankey_chm.pdf")), width=10, height=10) -->
